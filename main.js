...
var floor, lion, fan,
  ...
  mainScene, mainLion, mainRenderer, mainCamera;
...
function Fan() {
  ...
  var trans = new THREE.Matrix4().makeTranslation(0, 25, 0);
  if (propGeom.applyMatrix4) propGeom.applyMatrix4(trans); else propGeom.applyMatrix(trans);
...
function Lion() {
  ...
  var mustacheTrans = new THREE.Matrix4().makeTranslation(15, 0, 0);
  if (mustacheGeom.applyMatrix4) mustacheGeom.applyMatrix4(mustacheTrans); else mustacheGeom.applyMatrix(mustacheTrans);
  ...
  var kneeTrans = new THREE.Matrix4().makeTranslation(0, 50, 0);
  if (kneeGeom.applyMatrix4) kneeGeom.applyMatrix4(kneeTrans); else kneeGeom.applyMatrix(kneeTrans);
  ...
  this.bodyVertices = [];
  this.bodyInitPositions = [];
  var geom = this.body.geometry;
  var posAttr = geom && geom.attributes && geom.attributes.position;
  if (posAttr && posAttr.array) {
    var posArray = posAttr.array;
    var vertexCount = posArray.length / 3;
    this.bodyVertices = [0, 1, 2, 3, 4].filter(function (i) { return i < vertexCount; });
    if (vertexCount > 10) this.bodyVertices.push(10);
    for (var i = 0; i < this.bodyVertices.length; i++) {
      var vi = this.bodyVertices[i];
      var idx = vi * 3;
      posArray[idx + 2] = 70;
    ...
    if (posAttr.needsUpdate !== undefined) posAttr.needsUpdate = true;
  }
  ...
  for (var j = 0; j < 4; j++) {
    for (var k = 0; k < 4; k++) {
      ...
      var amp, zOffset, periodOffset = Math.random() * Math.PI * 2;
...
Lion.prototype.look = function (xTarget, yTarget) {
  ...
  if (this.bodyVertices.length > 0) {
    var posAttr = this.body.geometry && this.body.geometry.attributes && this.body.geometry.attributes.position;
    if (posAttr && posAttr.array) {
      var posArray = posAttr.array;
      for (var i = 0; i < this.bodyVertices.length; i++) {
        var tvInit = this.bodyInitPositions[i];
        var idx = this.bodyVertices[i] * 3;
        posArray[idx] = tvInit.x + this.head.position.x;
      }
      if (posAttr.needsUpdate !== undefined) posAttr.needsUpdate = true;
    }
  }
...
Lion.prototype.cool = function (xTarget, yTarget, deltaTime) {
  ...
  if (this.bodyVertices.length > 0) {
    var posAttrBody = this.body.geometry && this.body.geometry.attributes && this.body.geometry.attributes.position;
    if (posAttrBody && posAttrBody.array) {
      var posArray = posAttrBody.array;
      for (var i = 0; i < this.bodyVertices.length; i++) {
        var tvInit = this.bodyInitPositions[i];
        var idx = this.bodyVertices[i] * 3;
        posArray[idx] = tvInit.x + this.head.position.x;
      }
      if (posAttrBody.needsUpdate !== undefined) posAttrBody.needsUpdate = true;
    }
  }
};
...
function createMainScreen() {
  try {
    ...
    nearPlane = 1;
    farPlane = 2000;
    mainCamera = new THREE.PerspectiveCamera(fieldOfView, aspectRatio, nearPlane, farPlane);
    mainCamera.position.z = 800;
    ...
    var mainContainer = document.getElementById('world-main');
    mainContainer.appendChild(mainRenderer.domElement);

    var mainLight = new THREE.HemisphereLight(0xffffff, 0xffffff, 0.5);
    var mainShadowLight = new THREE.DirectionalLight(0xffffff, 0.8);
    mainShadowLight.position.set(200, 200, 200);
    mainShadowLight.castShadow = true;
    var mainBackLight = new THREE.DirectionalLight(0xffffff, 0.4);
    mainBackLight.position.set(-100, 200, 50);
    mainScene.add(mainBackLight);
    mainScene.add(mainLight);
    mainScene.add(mainShadowLight);

    mainLion = new Lion();
    mainScene.add(mainLion.threegroup);
    mainLion.look(0, 0);
    ...
    console.error('Main screen init error:', e);
  }
}
...
function start() {
  try {
    ...
    console.error('Chill the Lion init error:', e);
    var msg = (e && e.message) ? e.message : String(e);
    var worldEl = document.getElementById('world') || document.getElementById('world-main');
    if (worldEl) {
      worldEl.innerHTML =
        '<p style="padding:2rem;color:#653f4c;text-align:center;font-size:14px;">Не удалось загрузить сцену.<br><br><strong>Ошибка:</strong> ' + msg + '</p>';
    }
  }
}
